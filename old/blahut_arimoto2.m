function [R,V,Pa,Pas] = blahut_arimoto2(Ps,Q,b)
    
    % Blahut-Arimoto algorithm applied to the reward-complexity trade-off.
    %
    % USAGE: [R,V,Pa] = blahut_arimoto2(Ps,Q,[b])
    %
    % INPUTS:
    ...
    %   Pas - [K x S x A] conditional action policy %RB
    %
    ...
    R = zeros(length(b),1); V = zeros(length(b),1); Pa = zeros(length(b),A); Pas = zeros(length(b),S,A);
