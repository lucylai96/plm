    ...
    case 1
        ...
        for c = 1:length(cc)
            for i = 1:10 %:length(data) % for each subject
                ...
                figure(100+c); hold on; plot([data(s).lc.theta(1,2)],mean(data(s).lc.gobias),'r.','MarkerSize', 40)
                 plot([data(s).hc.theta(1,2)],mean(data(s).hc.gobias),'b.','MarkerSize', 40)
                ylabel('Go bias');xlabel('\theta_1')
                legend('LC','HC')
                prettyplot
                
                GO(c,i,1) = mean(data(s).lc.gobias);
                GO(c,i,2) = mean(data(s).hc.gobias);
                THETA(c,i,1) = data(s).lc.theta(1,2);
                 THETA(c,i,2) = data(s).hc.theta(1,2);
            end
            close all
        end
